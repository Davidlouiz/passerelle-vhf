...
class OpenWindMapProvider(WeatherProvider):
    ...
    async def fetch_measurement(self, station_id: str) -> Optional[Measurement]:
        ...
        try:
            ...
            raise ProviderError(f"Erreur HTTP lors de la récupération des données Pioupiou: {e}")
        except Exception as e:
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if isinstance(data, dict) and "data" in data:
                    for station_data in data["data"]:
                        ...
                        try:
                            all_measurements[station_id] = self._parse_measurement(station_data)
                        except:
    ...
    def _parse_measurement(self, data: dict) -> Optional[Measurement]:
        ...
        try:
            ...
            # Convertir la date
            date_str = data.get("date") or measurements.get("date")
            if date_str:
            ...
            return Measurement(
                ...
                wind_min_kmh=float(measurements.get("wind_speed_min")) if measurements.get("wind_speed_min") else None
            )
