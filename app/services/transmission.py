...
class TransmissionService:
    ...
    async def transmit(
        ...
        if not acquired:
            raise PTTError(f"Impossible d'acquérir le verrou TX (timeout {timeout_seconds}s)")
    ...
    async def _watchdog(self, timeout_seconds: int, start_time: datetime):
        ...
        logger.error(f"WATCHDOG: Timeout de {timeout_seconds}s atteint, forçage PTT OFF")
        try:
            self.ptt.set_ptt(False)
