...
class GPIOPTTController(PTTController):
    ...
    def __init__(self, pin: int, active_level: int = 1):
        ...
        logger.info(f"Contrôleur PTT GPIO initialisé (pin={pin}, active_level={active_level})")
    ...
    def set_ptt(self, active: bool):
        ...
        logger.debug(f"GPIO pin {self.pin} -> {'HIGH' if level else 'LOW'} (PTT {'ON' if active else 'OFF'})")
