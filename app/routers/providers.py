...
def update_credentials(
    ...
    if data.provider_id == "openwindmap":
        raise HTTPException(status_code=400, detail="OpenWindMap ne nécessite pas de credentials")
...
def resolve_station(
    ...
    if "balisemeteo.com" in url or "ffvl" in url.lower():
        provider_id, station_id, station_name = FFVLProvider.resolve_station_from_url(url)
    ...
    if "openwindmap.org" in url or "pioupiou" in url.lower():
        provider_id, station_id, station_name = OpenWindMapProvider.resolve_station_from_url(url)
        return StationResolutionResponse(
            provider_id=provider_id,
            station_id=station_id,
            station_name=station_name
